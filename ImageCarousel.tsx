...
const SingleExractor = (props: any) => {
  ...
  //   let Task: Array<ITasks> = FindInTasks(autoOcrResponse[0]?.tasks, pageNode?.taskId);
  //   let FoundPage: Array<IPagesData> = FindInPages(Task[0]?.pages, pageNode?.Page);
  //   let wordsList: Array<IWordsData> = FoundPage[0]?.words;
  ...
  const scroll = (direction: string) => {
    ...
    if (direction === "left") {
      if (pageData.index == 0) {
        ...
        handleScroll("e", pageList[parseInt(pageData.index) - 1], parseInt(pageData.index) - 1)
      }
    } else {
